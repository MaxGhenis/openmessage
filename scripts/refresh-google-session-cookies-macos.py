#!/usr/bin/env python3
"""Refresh OpenMessage Google account cookies from a local Chrome profile (macOS).

macOS sibling of refresh-google-session-cookies-linux.py, with the same CLI
contract (--quiet, --no-backup) so it can be used as
OPENMESSAGE_COOKIE_REFRESH_SCRIPT by the serve reconnect watchdog.

Differences from the Linux script:
- The Chrome Safe Storage secret comes from the macOS login keychain
  (`security find-generic-password`), not KWallet.
- AES-128-CBC decryption shells out to /usr/bin/openssl, so the script needs
  no third-party Python packages and can ship inside the app bundle.
- The cookie DB is copied (with its WAL) to a temp dir before reading, because
  a running Chrome holds the live DB and an immutable read can miss the most
  recent cookie rotation — Google session cookies rotate roughly every 30
  minutes, so staleness matters here.

It updates only auth_data.cookies in OpenMessage's session.json and never
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=os.environ.get(
            ...
            str(home / "Library/Application Support/OpenMessage/session.json"),
        ),
...
def aes_cbc_decrypt(ciphertext: bytes, key: bytes) -> bytes:
    """AES-128-CBC decrypt with IV of 16 spaces via /usr/bin/openssl (-nopad;
    PKCS7 padding is stripped by the caller so we can validate it)."""
...
def decrypt_cookie(encrypted_value: bytes, host: str, key: bytes) -> str | None:
    ...
    padded = aes_cbc_decrypt(encrypted_value[3:], key)
    if not padded:
        raise ValueError("empty decryption result")
    pad_len = padded[-1]
...
def snapshot_cookie_db(profile: Path) -> tuple[Path, tempfile.TemporaryDirectory]:
    """Copy the cookie DB (+WAL/SHM) so we read the freshest values even while
    Chrome holds the live file. Returns the copied DB path and the tempdir
    handle keeping it alive."""
    candidates = [profile / "Network" / "Cookies", profile / "Cookies"]
    src = next((p for p in candidates if p.exists()), None)
    if src is None:
        raise FileNotFoundError(f"Chrome cookie DB not found under: {profile}")

    tmpdir = tempfile.TemporaryDirectory(prefix="om-cookie-refresh-")
    dst = Path(tmpdir.name) / "Cookies"
...
def load_chrome_cookies(profile: Path, secret: bytes) -> tuple[dict[str, str], int, int, int]:
    cookie_db, tmpdir = snapshot_cookie_db(profile)
    try:
        # Opening the copy read-write replays the WAL into the main DB, so
        # recently rotated cookies are visible.
        rows = sqlite3.connect(cookie_db).execute(
            """
...
def main() -> int:
    args = parse_args()
    try:
        secret = chrome_safe_storage_secret()
